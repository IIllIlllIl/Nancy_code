...
client = OpenAI(api_key="fk721027-LMkJNp2jq5tAEE2wC2Mpc2EwHyNEgraY",
                base_url="http://chat.iteale.com:19017/v1")
import os
# from Sematic_err_correction import check_test_oracle_sematic
import sys
import run_llm_remote
...
def call_prompt_on_model(
        ...
        stop_w: str,
        max_tokens: int,
        code_model: str = "CodeLlama-13b-Python-hf",
...
def main():
    ...
    if len(arguments) != 7:
        raise SystemExit(
            '7 inputs are required: generate_test_oracle.py [prompt type] [dataset] [task_num] [script name] [output prefix name] [csv report filename (.csv)]')
    else:
